...
export function Robot(props) {
  // Load 3D model and materials using useGLTF
  const { nodes, materials } = useGLTF('./models/robot/phantoms-transformed.glb')

  // References for the robot group, scroll, and GSAP timeline
  const robot = useRef()
  const scroll = useScroll()
  const tl = useRef()

  // Update GSAP timeline on each frame based on scroll offset
  useFrame((state, delta) => {
    tl.current.seek(scroll.offset * tl.current.duration())
  })

  // Use layout effect to initialize GSAP timeline
  useLayoutEffect(() => {
  ...
  return (
    <group {...props} dispose={null} ref={robot}>
      <group position={[-0.21, 0.16, 0.37]} rotation={[0, 0, 0]} scale={0.15}>
        ...
        {/* Second cube with physical material */}
...
// Preload the 3D model to ensure it's loaded before rendering
